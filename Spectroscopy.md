...
さて、主成分分析を理解したところで、スペクトルデータに戻ります。サンプル数は88、1スペクトルのデータ数は2001です。

手順は以下の通りです。

- indexとしてサンプル名を、カラム名を波数としてpandasのDataFrameに読み込みます。
- scikit-learnのPCAクラス関数を呼び出し、第10成分までの主成分を計算します。
- 第10成分までの寄与率を棒グラフに表示します。
- 入力した二次微分曲線を表示します。
- 上に対応する因子負荷量を第3主成分まで表示します。
...
この場合、10次元まで表示しましたが、第３主成分まででサンプルのもつ分散のほぼ98％が説明されていることがわかります。よって、ここから先は３成分のみに対象をかぎって進めます。

それぞれのサンプルの主成分値は、2次微分スペクトルに係数hを掛けて線形和を求めたものですが、2001個のスペクトルデータそれぞれにかける$h$は$（固有値の平方根）・（固有ベクトル）$で与えられます。
